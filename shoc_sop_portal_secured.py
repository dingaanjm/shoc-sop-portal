...
def _svg_badge(label_top: str, label_bottom: str, color1: str, color2: str) -> str:
    ...
    <svg xmlns="http://www.w3.org/2000/svg" width="120" height="120" viewBox="0 0 120 120">
      ...
      <text x="60" y="52" text-anchor="middle" font-size="28" font-weight="700" fill="white" font-family="Arial">{label_top}</text>
      <text x="60" y="77" text-anchor="middle" font-size="18" font-weight="700" fill="white" font-family="Arial">{label_bottom}</text>
    </svg>
    '''
...
def render_preview_block(file_item: Optional[FileItem], key_prefix: str, default_expand: bool = False) -> None:
    ...
    with st.expander(f"Preview: {file_item.name}", expanded=default_expand):
        if file_item.ext == ".pdf":
            ...
            st.info("Preview is not supported for this file type inside the portal. Use Download.")
...
def render_dashboard(repo: List[SOPEntry]) -> None:
    ...
    if DEBUG_MODE:
        ...
        st.write("DEBUG BASE_DIR:", str(BASE_DIR))
        st.write("DEBUG MAIN_DIR exists:", MAIN_DIR.exists())
        st.write("DEBUG TOOLKIT_DIR exists:", TOOLKIT_DIR.exists())
        st.write("DEBUG TEMPLATES_DIR exists:", TEMPLATES_DIR.exists())
        st.write("DEBUG SOP count:", len(repo))
        st.write("DEBUG SOP names:", [x.name for x in repo])
...
def render_sop_detail(entry: SOPEntry) -> None:
    ...
    with tab1:
        ...
        if DEBUG_MODE:
            st.write("DEBUG - Main Word:", str(entry.main_word.path) if entry.main_word else "Not found")
            st.write("DEBUG - Main PDF:", str(entry.main_pdf.path) if entry.main_pdf else "Not found")
            st.write("DEBUG - Annexes:", [f.name for f in entry.annex_files])
        ...
        with w1:
            ...
            if entry.main_word:
                ...
                render_preview_block(entry.main_word, f"{entry.name}_main_word", default_expand=False)
            else:
        ...
        with w2:
            ...
            if entry.main_pdf:
                ...
                render_preview_block(entry.main_pdf, f"{entry.name}_main_pdf", default_expand=False)
            else:
    ...
    with tab2:
        ...
        if DEBUG_MODE:
            st.write("DEBUG - Expected toolkit folder:", str(TOOLKIT_DIR / entry.name))
            st.write("DEBUG - Toolkit files:", [f.name for f in entry.toolkit_files])
    ...
    with tab3:
        ...
        if DEBUG_MODE:
            st.write("DEBUG - Expected template folder:", str(TEMPLATES_DIR / entry.name))
            st.write("DEBUG - Template files:", [f.name for f in entry.template_files])
